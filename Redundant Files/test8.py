...
class Navigator:
    ...
    def execute_simple_turn(self, turn_command, request_id="debug"):
        """
        Executes turn: waits for CENTER sensor to leave initial line, then waits for ANY sensor to detect new line.
        """
        print(f"Executing sensor-based turn (Leave Center->Detect Any): {turn_command}")
        self.hw.send_command(turn_command)
        start_time = time.time()
        state = "LEAVING_CENTER" # State machine: LEAVING_CENTER -> SEARCHING_NEW_LINE

        while time.time() - start_time < self.turn_timeout:
            sensors = self.hw.read_ir_sensors(); left, center, right = sensors
            print(f"Turn State: {state}, Sensors: {left}{center}{right}") # Keep print uncommented

            if state == "LEAVING_CENTER":
                # Wait until center sensor is OFF the initial line (sees 1)
                if center == 1:
                    print("DEBUG: Center sensor left initial line (center == 1).")
                    state = "SEARCHING_NEW_LINE"
                # Keep turning

            elif state == "SEARCHING_NEW_LINE":
        ...
        self.hw.send_command(CMD_STOP); self.fb.update_trolley_status(request_id, f"error:turn_timeout:{turn_command}"); return False
