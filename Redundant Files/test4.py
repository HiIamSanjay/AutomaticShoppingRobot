...
- Includes 0.5s forward nudge before turning into aisles.
...
"""
Raspberry Pi Controller for Automated Shopping Trolley (Refactored Version)
... (Keep header comments, imports, constants, other classes the same) ...
- Turn logic now stops when *any* sensor detects the new line.
"""

# ... (Imports, Constants, HardwareInterface, LineFollower, FirebaseComm classes remain the same) ...


# --- Navigation Logic (execute_simple_turn MODIFIED) ---
class Navigator:
    ...
    def __init__(self, hw_interface: HardwareInterface, line_follower: LineFollower, firebase_comm: FirebaseComm):
        ...
        self.turn_timeout = 20 # Timeout for the turn execution
    ...
    def execute_simple_turn(self, turn_command, request_id="debug"):
        """
        Executes a turn (L or R) using sensor feedback:
        1. Starts turning.
        2. Tries to detect leaving the initial line (1 1 1), but forces progress after a few seconds.
        3. Continues turning until ANY sensor detects the new line (sees a '0').
        4. Stops turning. Centering will be handled by subsequent navigation.
        """
        print(f"Executing sensor-based turn (Stop on Any Detect, Forced Progress): {turn_command}")
        self.hw.send_command(turn_command)
        start_time = time.time()
        state = "STARTING_TURN"
        # Timeout specifically for trying to get off the initial line before forcing state change
        max_time_to_leave_line = 3.0 # seconds
        ...
        while time.time() - start_time < self.turn_timeout:
            ...
            # --- UNCOMMENTED SENSOR PRINT ---
            print(f"Turn State: {state}, Sensors: {left}{center}{right}")
            # ---

            if state == "STARTING_TURN":
                # Wait until we are clearly off the initial line segment (1 1 1)
                if left == 1 and center == 1 and right == 1:
                    print("DEBUG: Left initial line (1 1 1 detected).")
                    state = "LEFT_LINE"
                # --- ADDED: Force transition if stuck trying to leave line ---
                elif time.time() - start_time > max_time_to_leave_line:
                    print(f"DEBUG: Forcing transition to LEFT_LINE after {max_time_to_leave_line}s (did not see 1 1 1).")
                    state = "LEFT_LINE"
                # ---
                # Keep turning if still seeing parts of the initial line and internal timeout not reached

            elif state == "LEFT_LINE":
                # Now turning, looking for ANY sensor to detect the new line (a '0')
                if left == 0 or center == 0 or right == 0:
                    print(f"DEBUG: New line detected by at least one sensor ({left}{center}{right}). Stopping turn.")
                    self.hw.send_command(CMD_STOP) # Stop rotation
                    time.sleep(0.2) # Pause briefly
                    return True # Indicate turn procedure finished (line was found)
                # Keep turning if all sensors still see white (1 1 1)

            time.sleep(0.05) # Small delay in loop

        # --- Loop End (Overall Turn Timeout) ---
        print(f"ERROR: Turn timed out after {self.turn_timeout}s! Final state: {state}")
        # Ensure motors are stopped if timeout occurs
    ...
    def navigate_to_node(self, destination_node_name, request_id):
        ...
        # (Code remains the same as previous version with RFID debug prints)
        ...
        if initial_turn:
            if not self.execute_simple_turn(initial_turn, request_id):
                return False
            # After successful turn, robot is stopped. Start moving forward.
...
class TrolleyController:
    ...
    def _move_trolley_to_home(self, request_id):
        ...
        if current_node.startswith("pdt") or current_node.startswith("RBJ"):
            try:
                ...
                if current_node.startswith("RBJ"):
                     if not self.navigator.execute_simple_turn(CMD_RIGHT, request_id): path_ok = False
                if path_ok and not self.navigator.navigate_to_node(target_rfj, request_id): path_ok = False
            except Exception as e:
        ...
        if path_ok and current_node.startswith("RFJ"):
             ...
             if not self.navigator.execute_simple_turn(CMD_LEFT, request_id): path_ok = False
        if path_ok:
            print("Proceeding to 'home' node...")
            if not self.navigator.navigate_to_node("home", request_id): path_ok = False
        if path_ok:
             self.firebase_comm.update_trolley_status(request_id, "arrived_at:home")
             print("[Trolley] Arrived at home position.")
        else: print("[Trolley] Failed to return home.")
    ...
    # --- THIS METHOD IS MODIFIED (Added Forward Nudge) ---
    def _process_row(self, request_id, product_list, row_number, is_last_row):
        ...
        time.sleep(0.5) # User requested delay
        self.hw_interface.send_command(CMD_STOP) # Stop briefly before turning
        time.sleep(0.1) # Small pause after stop
        # --- END ADDED FORWARD NUDGE ---

        # 2. Turn into Aisle (Assume Right Turn at RFJ enters aisle)
        print(f"Turning right into row {row_number} aisle...")
        # Uses the modified execute_simple_turn which stops on any line detect
        ...
        for product_id in sorted_product_list:
            ...
            # NOTE: navigate_to_node will now immediately start line following corrections
