...
cred = credentials.Certificate("/home/pie/shopping_trolley/serviceAccountKey.json")
...
def follow_line():
    ...
    if not left and not center and not right: pass
    if not left and center and not right:   return "CENTERED"
    if left and not center and not right:   return "SLIGHT_RIGHT"
    if not left and not center and right:   return "SLIGHT_LEFT"
    if left and center and not right:      return "SLIGHT_RIGHT"
    if not left and center and right:      return "SLIGHT_LEFT"
    return "OTHER"

def handle_new_request(event):
    if event.data is None or event.path == "/": return
    print("New request:", event.data)
    request_id = event.path.split('/')[-1]
    process_shopping_list(request_id, event.data)

def move_trolley_to_home(request_id):
    print("[Trolley] Moving to home...")
    current_position = "home"
    status_ref = db.reference(f'trolleyStatus/{request_id}')
    success = navigate_to_node(current_position, "home", request_id)  # Get result
    if success: # check for success
        current_position = "home"
        status_ref.set("Trolley at home position")
        print("[Trolley] Arrived at home.")
    else:
        status_ref.set("error:homing_failed") # Set error status
        print("[Trolley] Error: Failed to reach home.")
...
def process_shopping_list(request_id, shopping_list):
    ...
    existing_cart = existing_cart_ref.get() or {}
    ...
    if isinstance(shopping_list, dict) and 'cart' in shopping_list:
        ...
        print("Error: Invalid list format or missing 'cart'.")
    ...
    listRow1, listRow2, listRow3 = [], [], []
    for product_id in existing_cart:
        row = PRODUCT_ROWS.get(product_id)
        if   row == 1: listRow1.append(product_id)
        elif row == 2: listRow2.append(product_id)
        elif row == 3: listRow3.append(product_id)
        else: print(f"Warning: Product {product_id} unassigned.")

    print(f"Rows - 1: {listRow1}, 2: {listRow2}, 3: {listRow3}")

    current_row = None
    if listRow1:
        other_rows = bool(listRow2 or listRow3)
        current_position, current_row = process_row(request_id, listRow1, current_position, 1, other_rows)
        if current_position is None: return
    if listRow2:
        other_rows = bool(listRow3)
        if current_position.startswith("RBJ") and int(current_position[-1]) < 2:
            navigate_to_node(current_position, "RBJ2", request_id)
            current_position = "RBJ2"
        current_position, current_row = process_row(request_id, listRow2, current_position, 2, other_rows)
        if current_position is None: return
    if listRow3:
        other_rows = bool(listRow2 or listRow1)
        if current_position.startswith("RBJ") and int(current_position[-1]) < 3:
            navigate_to_node(current_position, "RBJ3", request_id)
            current_position = "RBJ3"
        current_position, current_row = process_row(request_id, listRow3, current_position, 3, False)
        if current_position is None: return


    print("[Trolley] Returning to home...")
    status_ref.set("waiting_for_home_confirmation")
    wait_for_home_confirmation(request_id) #waits for proceed home confirmation
...
def process_row(request_id, product_list, current_position, row_number, other_rows_exist):
    ...
    if row_number == 1:
        row_front_junction, row_back_junction = "RFJ1", "RBJ1"
    elif row_number == 2:
        row_front_junction, row_back_junction = "RFJ2", "RBJ2"
    elif row_number == 3:
        row_front_junction, row_back_junction = "RFJ3", "RBJ3"
    else:
    ...
    if current_position != row_front_junction:
        ...
        if not navigate_to_node(current_position, row_front_junction, request_id):
            return None, None # Stop if navigation fails.
    ...
    for product_id in product_list:
        product_node_name = f"pdt{product_id[-1]}"
        ...
        print(f"  Seeking: {product_name} (Node {product_node_name})")
        status_ref.set(f"moving_to_node:{product_node_name}:{product_name}")
        if not navigate_to_node(current_position, product_node_name, request_id):
            return None, None # Stop if navigation fails.
        current_position = product_node_name

        print(f"    Prompting user for {product_name}...")
        wait_for_user_confirmation(request_id)
        print(f"    User added {product_name}.")
        status_ref.set(f"product_added:{product_id}")

    print(f"Moving to {row_back_junction}")
    if not navigate_to_node(current_position, row_back_junction, request_id):
        return None, None # Stop on navigation failure
    current_position = row_back_junction

    # --- Turning logic removed and added to navigate_to_node

    return current_position, row_number
...
def navigate_to_node(current_position_name, destination_node_name, request_id):
    ...
    if current_position_name == "home":
        if destination_node_name.startswith("RFJ"):
            ...
            while True:
                if not left and not center and not right: pass
                if not left and center and not right:   return "CENTERED"
                if left and not center and not right:   return "SLIGHT_LEFT"
                if not left and not center and right:   return "SLIGHT_RIGHT"
                if left and center and not right:      return "SLIGHT_LEFT"
                if not left and center and right:      return "SLIGHT_RIGHT"
            time.sleep(0.05)
            ...
            return False                                # --- Turn Logic (BEFORE main loop) ---
    # RFJ to pdt
    ...
    #going back home from RFJ
        ...
        send_command('F') # Go forward to home
        time.sleep(1.0)
        send_command("R")
        turning = True
        turn_direction = 'R'
        ...
        if esp32.in_waiting > 0:
            ...
            if response.startswith("RFID:"):
                ...
                if uid == expected_uid:
                    ...
                    send_command('S')  # Stop
                    return True # return True for successful navigation
