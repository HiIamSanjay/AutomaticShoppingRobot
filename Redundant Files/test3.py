...
class Navigator:
    ...
    def execute_simple_turn(self, turn_command, request_id="debug"):
        ...
        1. Starts turning.
        2. Waits until sensors show it's OFF the initial line.
        3. Continues turning while looking for the CENTER of the new line.
        """
        print(f"Executing sensor-based turn: {turn_command}")
        self.hw.send_command(turn_command)
        start_time = time.time()
        state = "STARTING_TURN" # State machine: STARTING_TURN -> LEFT_LINE -> CENTERING

        while time.time() - start_time < self.turn_timeout:
            sensors = self.hw.read_ir_sensors()
            left, center, right = sensors
            # NOTE: Using direct sensor values here, not LineFollower state,
            # as we need specific patterns (like all off line).
            # Assumes 0=Black, 1=White

            # print(f"Turn State: {state}, Sensors: {left}{center}{right}") # Verbose Debug

            if state == "STARTING_TURN":
                # Wait until we are clearly off the initial line segment
                # Looking for all sensors to be OFF the line (1 1 1)
                if left == 1 and center == 1 and right == 1:
                    print("DEBUG: Left initial line (1 1 1 detected).")
                    state = "LEFT_LINE"
                # Keep turning if still seeing parts of the initial line

            elif state == "LEFT_LINE":
                # Now we are turning in open space, looking for the new line center
                # Looking for the CENTERED pattern (1 0 1)
                if left == 1 and center == 0 and right == 1:
                    print("Turn complete (centered on new line).")
                    self.hw.send_command(CMD_FORWARD) # Stabilize forward
                    time.sleep(0.3) # Optional stabilization delay
                    return True
                # Keep turning if not centered yet

            # Optional: Add handling for unexpected sensor readings during turn?
    ...
    # --- navigate_to_node method remains the same ---
    # --- set_current_position method remains the same ---
    # ... (rest of Navigator class) ...

# --- Remainder of the code (HardwareInterface, LineFollower, FirebaseComm, TrolleyController classes, ---
# --- constants, and the __main__ block) stays the same.
