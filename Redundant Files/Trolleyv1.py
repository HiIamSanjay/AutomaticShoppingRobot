...
# --- Read IR Sensors (Raspberry Pi) using lgpio --- (AS PROVIDED)
def read_ir_sensors():
...
def navigate_to_node(current_position_name, destination_node_name, request_id):
    """Navigates to a specific node using RFID, and turns."""
    status_ref = db.reference(f'trolleyStatus/{request_id}')
    ...
    to_home = False #if going to home
    ...
    while True:
        ...
        if esp32.in_waiting > 0:
            ...
            if response.startswith("RFID:"):
                ...
                rfj_uids = [get_expected_uid(f"RFJ{i}") for i in range(1, 4) if get_expected_uid(f"RFJ{i}") is not None]
                rbj_uids = [get_expected_uid(f"RBJ{i}") for i in range(1, 4) if get_expected_uid(f"RBJ{i}") is not None]

                if uid in rfj_uids:
                    print(f"At RFJ junction: {uid}")
                    current_rfj_num = int(uid[-1])
                    ...
                    elif to_home:  # Check the to_home flag here
                        send_command("R") #changed to right
                        turning = True
                        turn_direction = "R" #chagned to right
                    ...
                    if destination_node_name.startswith("RFJ"):
                        ...
                        if dest_rfj_num > current_rbj_num: #comparing current RBJ with the required destination RBJ
                            send_command('L')
                            turning = True
                            turn_direction = 'L'
                ...
                elif expected_uid is not None: #removed  and uid != expected_uid:
        ...
        if turning:
          turning = False #for now no line following
          turn_direction = None
        ...
        #         send_command('N')  # Slight right, ESP32 handles
        #     elif response == "SLIGHT_RIGHT":
        #         send_command('M')  # Slight left, ESP32 handles
        # print(f"L:{left_sensor}, C:{center_sensor}, R:{right_sensor} - Turning:{turning}, Response:{response}") #for debuging

        # time.sleep(0.05)  # Short delay # removed line following
