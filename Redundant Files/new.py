...
def process_shopping_list(request_id, shopping_list):
   ...
   # --- Process each row sequentially ---
    ...
    send_command('R')
    turning = True
    turn_direction = 'R'  # Right turn at the RFJ to face home
...
def process_row(request_id, product_list, current_position, row_number, other_rows_exist):
    ...
    turning = True
    turn_direction = 'R'  # Right turn at the RFJ to face home
    #time.sleep(3.5)    # Delay for the turnc
    # ALWAYS turn ri00ght at RFJs
...
def navigate_to_node(current_position_name, destination_node_name, request_id):
    ...
    if current_position_name == "home":
        ...
        send_command('R')
        #time.sleep(1.5)  # Turn right from RFJ to product
        turning = True
        turn_direction = 'R'
    ...
    #going from pdt to pdt, or pdt to RBJ
        ...
        pass  # Go forward from pdt to pdt or RBJ
    ...
    #going from RBJ to RFJ
    elif current_position_name.startswith("RBJ") and destination_node_name.startswith("RBJ"):
        if int(destination_node_name[-1]) == 1: #for first row 
            pass
    ...
    #going back home from RFJ, no turns needed, will go straight F
        ...
        # Go forward to home <- NO, handled by main loop
    ...
    while True:
        ...
        if turning:
            if turn_direction == 'R':
                ...
                #print(left_sensor, center_sensor, right_sensor)
                if not left_sensor and center_sensor and not right_sensor:
                    turning = False  # Turn complete
                    turn_direction = None
                    send_command('F') #go forward
                    #time.sleep(0.5)  # Give it a little time to stabilize. Not in main loop
               ...
               if not left_sensor and center_sensor and not right_sensor: #back to center
                    ...
                    turn_direction = None
                    send_command('F')
                    #time.sleep(0.5) #give a little time to stabilize
