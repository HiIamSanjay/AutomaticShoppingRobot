...
def follow_line():
    ...
    if not left_sensor and center_sensor and not right_sensor:
        ...
        return "SLIGHT_LEFT"  # Corrected logic
    elif not left_sensor and center_sensor and right_sensor:
        return "SLIGHT_RIGHT"   # Corrected logic
    elif all([left_sensor, center_sensor, right_sensor]):
        return "ALL_BLACK" # Added all black case
    elif not any([left_sensor, center_sensor, right_sensor]):
      return "LOST"
    return "OTHER"  # Should ideally never reach here
...
def handle_new_request(event):
    ...
    if 'action' in shopping_list and shopping_list['action'] == 'home':
        ...
        processing_ref.delete()  # Delete after processing 'home'
        return

    process_shopping_list(request_id, shopping_list)
    processing_ref.delete()  # Delete after processing the shopping list
...
def move_trolley_to_home(request_id):
    ...
    # Assuming you start from home.  Important to set this!
    current_position = "home"
    navigate_to_node(current_position, "home", request_id)
    current_position = "home"  # Update current position
...
def process_shopping_list(request_id, shopping_list):
    ...
    if isinstance(shopping_list, dict) and 'cart' in shopping_list:
        ...
        for product_id, quantity in cart_data.items():
            ...
            if product_id in ('processed', 'action'):  # Skip 'processed' and 'action'
    ...
    current_row = None  # Keep track of current row
    if listRow1:
        # Check if other rows have items *before* processing row 1
        other_rows_exist = bool(listRow2 or listRow3)
    ...
    if listRow3:
        ...
        current_position, current_row = process_row(request_id, listRow3, current_position, 3, False)  # No rows after
        if current_position is None: return
...
def process_row(request_id, product_list, current_position, row_number, other_rows_exist):
    ...
    if current_position != row_front_junction:
        ...
        navigate_to_node(current_position, row_front_junction, request_id)
    ...
    for product_id in product_list:
        ...
        navigate_to_node(current_position, product_node_name, request_id)
    ...
    navigate_to_node(current_position, row_back_junction, request_id)
...
def wait_for_user_confirmation(request_id):
    ...
    while True:
        if confirmation_ref.get() is True:
            confirmation_ref.set(False)  # Reset for next use
...
def navigate_to_node(current_position_name, destination_node_name, request_id):
    ...
    if current_position_name == "home" and destination_node_name.startswith("RFJ"):
      ...
      while True:
          ...
          if not left and center and not right:
              print("Centered.")
              send_command('F')
              time.sleep(0.75)  # Adjust delay as needed
              break  # Exit centering loop
          elif (left and not center) or (left and center and not right):
              send_command('N')
              time.sleep(0.05)
          elif (not left and not center and right) or (not left and center and right):
            send_command('M')
            time.sleep(0.05)
          elif all([left, center, right]):  # All on black
    ...
    # RFJ to pdt
        ...
        #send_command('F') # move forward a bit  <- REMOVED extra forward
        #time.sleep(1.0)  # delay  <- REMOVED extra delay
        send_command('R')  # Turn right from RFJ to product
        time.sleep(0.5) # Added delay after turning command.
        turning = True
        turn_direction = 'R'
    ...
    while True:
        ...
        if esp32.in_waiting > 0:
            ...
            if response.startswith("RFID:"):
                ...
                if uid == expected_uid:
                    ...
                    status_ref.set(f"waiting_for_rfid:{destination_node_name}")  # Moved inside the RFID check
                    send_command('S')  # Stop
        ...
        if turning:
            if turn_direction == 'R':
                ...
                if not left_sensor and center_sensor and not right_sensor:
                    ...
                    send_command("F")  # Move forward after turn
                    time.sleep(0.5)
        ...
        # --- 3. Line Following (if not turning) ---
            ...
            if response == "LOST":
                send_command('S')
                print("Line Lost!")
                return  # Indicate failure, exit function
               ...
               send_command("F")
                ...
                time.sleep(0.05)
            elif response == "ALL_BLACK":  # All sensors on black
